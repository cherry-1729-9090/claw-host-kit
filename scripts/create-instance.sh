...
OPENCLAW_COMPOSIO_API_KEY="${OPENCLAW_COMPOSIO_API_KEY:-${COMPOSIO_API_KEY:-}}"
OPENCLAW_FIRECRAWL_API_KEY="${OPENCLAW_FIRECRAWL_API_KEY:-${FIRECRAWL_API_KEY:-}}"
...
HOSTNAME="openclaw-${DNS_INSTANCE_ID}.${WILDCARD_DOMAIN}"
...
PIDS_LIMIT="${OPENCLAW_PIDS_LIMIT:-2048}"

DOCKER_ENV_ARGS=()
if [ -n "${OPENCLAW_MEM0_API_KEY}" ]; then
  DOCKER_ENV_ARGS+=(-e "MEM0_API_KEY=${OPENCLAW_MEM0_API_KEY}")
fi
if [ -n "${OPENCLAW_COMPOSIO_API_KEY}" ]; then
  DOCKER_ENV_ARGS+=(-e "OPENCLAW_COMPOSIO_API_KEY=${OPENCLAW_COMPOSIO_API_KEY}")
  DOCKER_ENV_ARGS+=(-e "COMPOSIO_API_KEY=${OPENCLAW_COMPOSIO_API_KEY}")
fi
if [ -n "${OPENCLAW_FIRECRAWL_API_KEY}" ]; then
  DOCKER_ENV_ARGS+=(-e "OPENCLAW_FIRECRAWL_API_KEY=${OPENCLAW_FIRECRAWL_API_KEY}")
  DOCKER_ENV_ARGS+=(-e "FIRECRAWL_API_KEY=${OPENCLAW_FIRECRAWL_API_KEY}")
fi
